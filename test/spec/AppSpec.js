describe('App', function() {
  ...
  it('queues the next song when an "enqueue" event is fired', function(){ 
    app.get('library').at(1).enqueue(); // fire enqueue() on library[1] (next song)....
    expect(app.get('songQueue').at(0)).to.equal(app.get('library').at(1)); // check whether songQueue[0] is library[1].
  });
